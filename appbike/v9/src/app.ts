...
export class App {
    ...
    async registerUser(user: User): Promise<string> {
        if (await this.userRepo.find(user.email)) throw new DuplicateUserError()
    ...
    async removeUser(email: string): Promise<void> {
        ...
        if(await this.rentRepo.findOpenRentsFor(email)) throw new OpenRentsError()
    ...
    async returnBike(bikeId: string, userEmail: string): Promise<number> {
        ...
        rent.end = now
        await this.bikeRepo.update(rent.id, rent.bike)
        rent.bike.available = true
        await this.bikeRepo.update(bikeId, rent.bike)
    ...
    async findBike(bikeId: string): Promise<Bike> {
        const bike = this.bikeRepo.find(bikeId)
        if (!bike) throw new BikeNotFoundError()
